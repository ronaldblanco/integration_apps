...
<table class="table">
    ...
    'crm.timeline.comment.list' ,
		...
		//$row["ASSIGNED_BY_ID"] = $responsable['result']['NAME'] . ' ' . $responsable['result']['LAST_NAME'];
		//var_dump($timeline);
...
echo $data;*/
    ...
    <a class="nav-link active" id="comment-tab" data-toggle="tab" href="#comment" role="tab" aria-controls="comment"
      aria-selected="true">Comment</a>
					...
					<li class="nav-item">
    <a class="nav-link" id="CALL-tab" data-toggle="tab" href="#CALL" role="tab" aria-controls="CALL"
    ...
    <a class="nav-link" id="CALENDAR-tab" data-toggle="tab" href="#CALENDAR" role="tab" aria-controls="CALENDAR"
      aria-selected="false">Meeting</a>
				...
				<div class="tab-pane fade show active" id="comment" role="tabpanel" aria-labelledby="comment-tab"></br></br>
					<form action="appCOMMENTRedirect.php" method="get" styles>
						<div class="form-group">
							<label>Comment:</label>
							<input type="hidden" class="form-control" name="fields[ENTITY_ID]" value='<?php echo $row['contactid'];?>'>
							<input type="hidden" class="form-control" name="fields[ENTITY_TYPE]" value='contact'>
        					<textarea class="form-control" name="fields[COMMENT]" placeholder="Comment to post" value='' rows="5" style="width: 100%"></textarea>
							<small id="comment" class="form-text text-muted">Comment to post!</small>
						</div>
        				<input type="submit" class="btn btn-primary" value="Post">
    				</form></br>
				</div>
				<div class="tab-pane fade" id="SMS" role="tabpanel" aria-labelledby="SMS-tab"></br></br>
					<form action="<?php echo $env['smshandler'];?>" method="post" styles>
						<div class="form-group">
							...
							<textarea class="form-control" name="message_body" placeholder="SMS text" value='' rows="5" style="width: 100%"></textarea>
							<small id="SMS" class="form-text text-muted">SMS Text to send!</small>
						</div>
        				<input type="submit" class="btn btn-primary" value="Send">
    				</form></br>
				</div>
				<div class="tab-pane fade" id="CALL" role="tabpanel" aria-labelledby="CALL-tab"></br></br>
					<form action="<?php echo $env['callhandler'];?>?ext=78&callto=1234567899" method="post" styles>
						<div class="form-group">
							<label>CALL:</label>
							<input type="hidden" class="form-control" name="data[CRM_ENTITY_TYPE]" value='contact'>
							<input type="hidden" class="form-control" name="data[USER_ID]" value='<?php echo $row['IDresponsable'];?>'>
							<input type="hidden" class="form-control" name="data[PHONE_NUMBER]" value='<?php echo $row['contactphone'];?>'>
							<input type="hidden" class="form-control" name="data[CRM_ENTITY_ID]" value='<?php echo $row['contactid'];?>'>
							<input type="hidden" class="form-control" name="auth[client_endpoint]" value='https://<?php echo $env['bitrix_domain'];?>/rest/'>
							<input type="hidden" class="form-control" name="auth[domain]" value='<?php echo $env['bitrix_domain'];?>'>
							<input type="hidden" class="form-control" name="auth[member_id]" value='<?php echo $env['member_id'];?>'>
							<input type="hidden" class="form-control" name="auth[application_token]" value='<?php echo $env['application_token'];?>'>
							<input type="hidden" class="form-control" name="redirect" value='<?php echo $env['integrationrediredurl'];?>appCALLRedirect.php?contactID=<?php echo $row['contactid'];?>&coordinatorID=<?php echo $row['IDresponsable'];?>'>
							<small id="CALL" class="form-text text-muted">CALL to contact!</small>
						</div>
        				<input type="submit" class="btn btn-primary" value="Call">
    				</form></br>
				</div>
				<div class="tab-pane fade" id="TASK" role="tabpanel" aria-labelledby="TASK-tab"></br></br>
					<form action="appTASKRedirect.php" method="get" styles>
						<div class="form-group">
							<label>Task:</label>
							<input type="text" class="form-control" name="fields[TITLE]" value='' placeholder='Title of Event'>
							<input type="hidden" class="form-control" name="fields[RESPONSIBLE_ID]" value='<?php echo $row['IDresponsable'];?>'>
							<small id="TASK" class="form-text text-muted">ADD Task!</small>
						</div>
        				<input type="submit" class="btn btn-primary" value="ADD">
    				</form></br>
				</div>
				<div class="tab-pane fade" id="CALENDAR" role="tabpanel" aria-labelledby="CALENDAR-tab"></br></br>
					<form action="appCALENDARRedirect.php" method="post" styles>
						<div class="form-group">
							<label>Meeting:</label>
							<input type="hidden" class="form-control" name="access_token" value='<?php echo $arAccessParams['access_token'];?>'>
							<input type="hidden" class="form-control" name="contactid" value='<?php echo $row['contactid'];?>'>
							<input type="hidden" class="form-control" name="IDresponsable" value='<?php echo $row['IDresponsable'];?>'></br>
							<label>From:</label>
							<input type="date" class="form-control" name="from" value=''>
							<label>To:</label>
							<input type="date" class="form-control" name="to" value=''>
							<label>Name:</label>
							<input type="text" class="form-control" name="name" value='' placeholder="Meeting Name">
							<label>Description:</label>
							<textarea class="form-control" name="description" placeholder="Meeting Description" value='' rows="3" style="width: 100%"></textarea>
							<small id="CALENDAR" class="form-text text-muted">ADD Meeting Event!</small>
						</div>
        				<input type="submit" class="btn btn-primary" value="ADD">
    				</form></br>
				</div>
