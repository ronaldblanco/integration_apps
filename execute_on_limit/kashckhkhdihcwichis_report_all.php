...
Execute task
*/
$sql = "SELECT from_num,domain,CONVERT_TZ(date_time,'+00:00','-05:00') as date FROM `execute_on_limit` where date_time LIKE '".$day."%' and CHAR_LENGTH(execute_on_limit.from_num) > 6 and from_num not LIKE 'line_%' order by date_time desc";
  ...
  //echo "Successfully";
	...
	if ($result->num_rows > 0) {
  	...
  	while($row = $result->fetch_assoc()) {
		...
		array_push($daily_report,["from_num" => $row["from_num"], "domain" => $row["domain"], "date" => $row["date"]]);
  	}
	} else {
  	//echo "0 results";
	}
...
<body>
	...
	<center><h2>Count of limit calls by Domain per day!</h2>

	<form action="https://integration.swaypc.com/execute_on_limit/kashckhkhdihcwichis_report_all.php" method="POST">
...
?>
	...
	<h4>Total limit Calls by Domain for the day: <?php echo $alltotal;?></h4>
