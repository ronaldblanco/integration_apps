...
Execute task
*/
$sql = "SELECT count(id) as count,from_num,domain,max(date_time) as date FROM `execute_on_limit` where date_time LIKE '" . $month . "%' and CHAR_LENGTH(execute_on_limit.from_num) > 6 and from_num not LIKE 'line_%' group by execute_on_limit.from_num order by count desc";
...
<body>
    ...
    <center>
        <h2>Limited calls number/month</h2>

        <form action="<?php echo $env['reportdetailsmonthlyurl'] . (isset($selectedDomain) ? "?domain=" . $selectedDomain : ""); ?>" method="POST">
            <p>Month</p> <!--<input type="text" name="day" value="<?php echo $day ?>">-->
            <input type="text" id="datepicker" name="month" value="<?php echo $month ?>"></p>
    ...
    <h4>Summary limited calls numbers/month: <?php echo $alltotal; ?></h4>
    ...
    <script>
	 ...
	 <script type="text/javascript">
		...
		if("<?php echo $width;?>" == "" || "<?php echo $width;?>" != width) window.location.replace('ajsbchcbkwdhcwdhbwhbcw_details_monthly.php?1=1<?php echo (isset($selectedDomain)?"&domain=".$selectedDomain:"").(isset($month)?"&month=".$month:"")?>&width='+width); 
